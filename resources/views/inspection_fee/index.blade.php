...
<div class="row">
    <div class="col-md-12 text-right" style="padding-right: 30px;">
        <a href="{{ route('inspection_fee.create') }}" class="btn btn-primary"><em class="fa fa-plus"></em><span>Add New Inspection Fee</span></a>
    </div>
</div>
<div class="content px-3" style="margin-bottom: 30px;">
   ...
   <div class="card">
    <div class="card-body p-5">
        <form method="GET" action="{{ route('inspection_fee.index') }}">
        ...
        <div class="table-responsive">
            <table class="table align-middle gs-0 gy-4" id="order-listing123">
                    ...
                    <tbody> @php
                        $no = 1;
                    @endphp
                        @foreach ($inspection_fees as $index => $inspection_fee)
                        <tr>
                            <td>
                                <input type="checkbox" class="checkbox-item" data-id="{{ $inspection_fee->id }}">
                            </td>
                            <td>{{ ucwords(strtolower($inspection_fee->service->name ?? 'NILL')) }}</td>
                                <td>{{ number_format($inspection_fee->amount, 2) }}</td>
                                <td>{{ ucwords(strtolower($inspection_fee->processingType->name ?? 'NILL')) }}</td>
                                <td>
                                    <a style="padding-right:10px;" href="{{ route('inspection_fee.edit', $inspection_fee->id) }}" title="Edit Service"><span
                                            class="nk-menu-icon text-info"><em class="fa fa-edit"></em></span></a>
                                    

                                            <a title="Terminate Item" style="cursor: pointer;"
                                            onclick="confirmDelete({{ $inspection_fee->id }});">
                                            <span class="nk-menu-icon text-danger eg-swal-av3"><em class="fa fa-trash"></em></span>
                                        </a>
                                        <form id="delete-item-form-{{ $inspection_fee->id }}" action="{{ route('inspection_fee.destroy', $inspection_fee->id) }}"
            ...
            <div>{{ $inspection_fees->appends(['search' => request('search')])->links() }}</div>
        </div> </div> </div>
    ...
    function deleteSelected() {
        ...
        if (selectedIds.length > 0) {
            ...
            if (confirmation) {
                ...
                $.ajax({
                    url: "{{ route('delete.selected.inspection.fees') }}",
                    type: 'DELETE',
                    data: {
        _token: '{{ csrf_token() }}',
