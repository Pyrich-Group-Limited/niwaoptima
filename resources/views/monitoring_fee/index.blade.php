...
<div class="content px-3" style="margin-bottom: 40px;">
   ...
   <div class="card">
    <div class="card-body p-5">
        <form method="GET" action="{{ route('monitoring_fee.index') }}">
        ...
        <div class="table-responsive">
            <table class="table align-middle gs-0 gy-4" id="order-listing123">
                    <thead>
                        <tr>
                            ...
                            <th>Monitoring Fee Name</th>
                            <th>Processing Type</th>
                            <th>Manage</th>
                        </tr>
                    </thead>
                    <tbody> 
                        @foreach ($monitoring_fees  as $index =>  $monitoring_fee)
                        <tr class="">
                            <td>
                                <input type="checkbox" class="checkbox-item" data-id="{{ $monitoring_fee->id }}">
                            </td>
                            <td>{{ ucwords(strtolower($monitoring_fee->service->name ?? 'NILL')) }}</td>
                             <td>{{ "₦".number_format($monitoring_fee->amount, 2) }}</td>
                               <td>{{ $monitoring_fee->name ? $monitoring_fee->name : '' }}</td>
                                <td>{{ $monitoring_fee->processingType->name ?? 'NILL' }}</td>
                                <td>
                                    <a style="padding-right:10px;" href="{{ route('monitoring_fee.edit', $monitoring_fee->id) }}" title="Edit Service"><span
                                            class="nk-menu-icon text-info"><em class="fa fa-edit"></em></span></a>
                                    

                                            <a title="Terminate Item" style="cursor: pointer;"
                                            onclick="confirmDelete({{ $monitoring_fee->id }});">
                                            <span class="nk-menu-icon text-danger eg-swal-av3"><em class="fa fa-trash"></em></span>
                                        </a>
                                        <form id="delete-item-form-{{ $monitoring_fee->id }}" action="{{ route('monitoring_fee.destroy', $monitoring_fee->id) }}"
            ...
            <div>{{ $monitoring_fees->appends(['search' => request('search')])->links() }}</div>
        </div> </div>
    ...
    function deleteSelected() {
        ...
        if (selectedIds.length > 0) {
            ...
            if (confirmation) {
                ...
                $.ajax({
                    url: "{{ route('delete.selected.monitoring.fees') }}",
                    type: 'DELETE',
                    data: {
        _token: '{{ csrf_token() }}',
